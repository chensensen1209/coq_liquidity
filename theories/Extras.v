...
(* 它接受一个函数 f 和一个列表 l 作为参数，并尝试在列表中查找第一个满足条件的元素 *)
Fixpoint find_first {A B : Type} (f : A -> option B) (l : list A)
  ...
  (* 将函数 f 应用于列表中的每个元素，将非空的结果构成一个新的列表返回。 *)
...
(*   
  ...
  Definition string_to_option_int (s : string) : option nat :=
  match s with
  | EmptyString => None
  | String _ _ => Some (length s) (* 将字符串长度作为整数返回 *)
  end.
  Compute (map_option string_to_option_int ["hello"%string; ""%string; "world"%string]).
  print [5; 5]: list nat *)
